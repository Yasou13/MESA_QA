...
async def test_tester_prompt_is_rendered_without_formatting_json_braces(tmp_path, kind):
    ...
    event = ScenarioEvent(id="evt-1", kind=kind, entity="project:atlas", field="backend", value="FastAPI")
...
async def test_tester_codex_failure_is_not_replaced_by_direct_mcp(tmp_path):
    ...
    tester = TesterCodex(runner=runner, prompts_dir=Path(__file__).parents[2] / "prompts")
