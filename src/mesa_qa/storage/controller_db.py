...
class ControllerDB:
    ...
    async def get_run_state(self, run_id: str) -> Optional[Dict[str, Any]]:
        async with aiosqlite.connect(self.db_path) as db:
            ...
            async with db.execute("SELECT * FROM run_state WHERE run_id = ?", (run_id,)) as cursor:
    ...
    async def record_bug(self, bug_id: str, run_id: str, severity: str, category: str, bug_data: Dict[str, Any], status: str, created_at: str) -> None:
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute("""
            ...
            """, (bug_id, run_id, severity, category, json.dumps(bug_data), status, created_at))
            await db.commit()
